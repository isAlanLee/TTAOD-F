...
class WindowMSA(BaseModule):
    ...
    def forward(self, x, mask=None, prompt_type=None, num_tokens=0):
        """
        Args:
        ...
        if prompt_type == "prepend":
            ...
            relative_position_bias = torch.cat((
                torch.zeros(_C, num_tokens, _W, device=attn.device),
                relative_position_bias
                ), dim=1)
            relative_position_bias = torch.cat((
                torch.zeros(_C, _H + num_tokens, num_tokens, device=attn.device),
                relative_position_bias
                ), dim=-1)
        ...
        if mask is not None:
            ...
            if prompt_type == "prepend":
                # expand relative_position_bias
                mask = torch.cat((
                    torch.zeros(nW, num_tokens, _W, device=attn.device),
                    mask), dim=1)
                mask = torch.cat((
                    torch.zeros(
                        nW, _H + num_tokens, num_tokens,
                        device=attn.device),
                    mask), dim=-1)
...
class ShiftWindowMSA(BaseModule):
    ...
    def forward(self, query, hw_shape, prompt_type=None, num_tokens=0):
        B, L, C = query.shape
        H, W = hw_shape

        if prompt_type == "prepend":
            prompt_emb = query[:, :num_tokens, :]
        ...
        if prompt_type == "prepend":
            ...
            prompt_emb = prompt_emb.reshape((-1, num_tokens, C))
            query_windows = torch.cat((prompt_emb, query_windows), dim=1)
        attn_windows = self.w_msa(query_windows, mask=attn_mask, 
                                  prompt_type=prompt_type, num_tokens=num_tokens)

        if prompt_type == "prepend":
            # seperate prompt embs --> nW*B, num_prompts, C
            # change input size
            prompt_emb = attn_windows[:, :num_tokens, :]
            attn_windows = attn_windows[:, num_tokens:, :]
            # change prompt_embs's shape:
            # nW*B, num_prompts, C - B, num_prompts, C
            prompt_emb = prompt_emb.view(-1, B, num_tokens, C)
...
class SwinBlock(BaseModule):
    ...
    def forward(self, x, hw_shape, prompt_type=None, num_tokens=0):

        def _inner_forward(x):
            identity = x
            x = self.norm1(x)
            x = self.attn(x, hw_shape, prompt_type, num_tokens)
...
class SwinBlockSequence(BaseModule):
    ...
    def forward(self, x, hw_shape, 
                prompt_type=None, num_tokens=0, prompt_deep=False, deep_prompt_embd=None, prompt_dict=None, stage_i=None):
        if not prompt_deep:
            for b_i, block in enumerate(self.blocks):
                if prompt_dict != None:
                    # 使用图像token的均值来初始化prompt
                    if stage_i == 0 and b_i == 0:
                        dict_n = "prompt_embeddings"
                        print(prompt_dict[dict_n].shape, x.mean(dim=1, keepdims=True).shape)
                        prompt_dict[dict_n] = x.mean(dim=1, keepdims=True)
                    elif stage_i == 0 and b_i == 1:
                        dict_n = "deep_prompt_embeddings_0"
                        print(prompt_dict[dict_n].shape, x.mean(dim=1, keepdims=True).shape)
                        prompt_dict[dict_n] = x.mean(dim=1, keepdims=True)                   
                    else:
                        dict_n = "deep_prompt_embeddings_{}".format(stage_i)
                        print(prompt_dict[dict_n][b_i].shape, x.mean(dim=1).shape)
                        prompt_dict[dict_n][b_i] = x.mean(dim=1)

                x = block(x, hw_shape, prompt_type, num_tokens)
        else:
            ...
            if deep_prompt_embd.shape[0] != num_blocks:
                ...
                for i in range(num_blocks):
                    if i == 0:
                        x = self.blocks[i](x, hw_shape, prompt_type, num_tokens)

                    else:
                        prompt_emb = deep_prompt_embd[i-1].expand(B, -1, -1)
                        x = torch.cat(
                            (prompt_emb, x[:, num_tokens:, :]),
                            dim=1
                        )
                        x = self.blocks[i](x, hw_shape, prompt_type, num_tokens)
            else:
        ...
        if self.downsample:
            x_down, down_hw_shape = self.downsample(x, hw_shape, prompt_type, num_tokens)
...
class SwinTransformer(BaseModule):
    ...
    def __init__(self,
        ...
        # vpt
        self.prompt_type = prompt_type    # "prepend" / "None"
        self.num_tokens = num_tokens      # 10
        self.prompt_deep = prompt_deep    # True / False
        ...
        if self.TTWS_init:
            self.prompt_dict = {
                ...
                "deep_prompt_embeddings_0": nn.Parameter(torch.zeros(
                                                        depths[0] - 1, 1, embed_dims
                                                        )), 
                "deep_prompt_embeddings_1": nn.Parameter(torch.zeros(
                                                        depths[1], 1, embed_dims * 2
                                                        )), 
                "deep_prompt_embeddings_2": nn.Parameter(torch.zeros(
                                                        depths[2], 1, embed_dims * 4
                                                        )),
                "deep_prompt_embeddings_3": nn.Parameter(torch.zeros(
                                                        depths[3], 1, embed_dims * 8
                                                        )),
            }
        ...
        if (self.prompt_type == "prepend" and self.TTWS_file is not None
            ...
            self.deep_prompt_embeddings_1 = nn.Parameter(prompt_init_dict["deep_prompt_embeddings_1"].repeat(1, self.num_tokens, 1))
